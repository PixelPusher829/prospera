      ...
      cell: (transaction) => (
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h4 className="overflow-hidden text-sm font-medium text-ellipsis whitespace-nowrap text-slate-700 dark:text-white">
              {transaction.payee}
            </h4>
            <p className="overflow-hidden text-xs text-ellipsis whitespace-nowrap text-slate-500 dark:text-slate-400">
    ...
    },
    {
      ...
      cell: (transaction) => (
        <div className="relative inline-block">
          <span className="max-w-xs overflow-hidden rounded border border-slate-200 bg-slate-100 px-2 py-1 text-xs text-ellipsis whitespace-nowrap text-slate-600 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300">
            {transaction.category}
          </span>
          <select
            value={transaction.category}
            onChange={(e) => onCategoryChange(transaction.id, e.target.value)}
            className="absolute inset-0 top-1 h-full w-full cursor-pointer text-sm opacity-0"
          >
