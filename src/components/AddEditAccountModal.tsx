...
import { X, Save, Landmark, CreditCard, ShieldCheck, TrendingUp, DollarSign, CheckCircle, Trash2 } from 'lucide-react';
...
const AddEditAccountModal: React.FC<AddEditAccountModalProps> = ({ isOpen, onClose, onSaveAccount, defaultAccountType, account, onDeleteAccount }) => {
  ...
  const [selectedAccountType, setSelectedAccountType] = useState<AccountType | 'Investment' | 'Loan'>(defaultAccountType); // New state for selected type
  const [selectedBankBranch, setSelectedBankBranch] = useState('Bank of America'); // New state for bank branch
  const [errors, setErrors] = useState<{ [key: string]: string }>({});
  const [isAccountSyncing, setIsAccountSyncing] = useState(false); // State for individual account sync
  const [showAccountSyncNotification, setShowAccountSyncNotification] = useState(false); // State for individual account sync notification
  const [selectedInvestmentCategory, setSelectedInvestmentCategory] = useState(account?.investmentCategory || INVESTMENT_CATEGORIES[0].value);
  ...
  useEffect(() => {
    if (isOpen) {
      if (account) {
        ...
        setSelectedAccountType(account.type === 'Investment' ? 'Investment' : account.type === 'Loan' ? 'Loan' : account.type as AccountType); // Set type from existing account
  ...
  const validate = () => {
    ...
    if (!currentAccount.balance && selectedAccountType === 'Cash') { // Only validate balance for Cash accounts if editable
      newErrors.balance = 'Balance is required.';
    } else if (selectedAccountType === 'Cash' && isNaN(parseFloat(currentAccount.balance))) {
      newErrors.balance = 'Balance must be a number.';
    } else if (selectedAccountType === 'Cash' && parseFloat(currentAccount.balance) === 0) {
  ...
  const handleSaveAccount = () => {
    ...
    const accountToSave: Account = {
      ...
      institution: selectedAccountType === 'Cash' ? '' : (account && account.institution ? account.institution : selectedBankBranch), // Use selectedBankBranch for non-cash new accounts, else keep existing or empty
      balance: parseFloat(currentAccount.balance) || parseFloat(generateRandomAmount()), // Use current balance if provided, else random
      type: selectedAccountType === 'Investment' ? 'Investment' : selectedAccountType === 'Loan' ? 'Loan' : (selectedAccountType as AccountType), // Cast to AccountType
      accountNumber: currentAccount.accountNumber || generateRandomAccountNumber(), // Use existing or generate random
      colorTheme: selectedAccountType === 'Investment' ? INVESTMENT_CATEGORIES.find(cat => cat.value === selectedInvestmentCategory)?.colorTheme : currentAccount.colorTheme || undefined, // Set colorTheme based on investment category if Investment, else use current
      investmentCategory: selectedAccountType === 'Investment' ? selectedInvestmentCategory : undefined, // Add investment category if Investment
      iconBgClass: selectedAccountType === 'Investment' ? INVESTMENT_CATEGORIES.find(cat => cat.value === selectedInvestmentCategory)?.iconBgClass : undefined, // Add icon background class if Investment
      iconTextColorClass: selectedAccountType === 'Investment' ? INVESTMENT_CATEGORIES.find(cat => cat.value === selectedInvestmentCategory)?.iconTextColorClass : undefined, // Add icon text color class if Investment
  ...
  const handleSyncIndividualAccount = () => {
    ...
    setIsAccountSyncing(true);
          setShowAccountSyncNotification(false);    setTimeout(() => {
      console.log('First setTimeout complete. Setting isAccountSyncing to false, showIndividualSyncNotification to true.');
  ...
  const renderFields = () => {
      ...
      const commonFields = (
        <>
          <div>
            ...
            <input type="text" name="name" value={currentAccount.name} onChange={handleInputChange} className={`w-full px-3 py-2 border rounded-lg transition-colors bg-transparent ${errors.name ? 'border-red-500' : 'border-slate-200'} ${account ? 'bg-slate-50 text-slate-500' : ''}`} placeholder="e.g., Everyday Checking" readOnly={!!account && selectedAccountType !== 'Cash' && selectedAccountType !== 'Credit' && selectedAccountType !== 'Loan' && selectedAccountType !== 'Investment'} />
          ...
          {(!selectedAccountType || selectedAccountType !== 'Cash') && account && ( // Render Sync Account button only if non-Cash and editing
            <div>
              <button
        ...
        return (
          <>
            <div>
              ...
              <input type="text" name="name" value={currentAccount.name} onChange={handleInputChange} className={`w-full px-3 py-2 border rounded-lg transition-colors bg-transparent ${errors.name ? 'border-red-500' : 'border-slate-200'} ${!!account ? 'bg-slate-50 text-slate-500' : ''}`} placeholder="e.g., Chase Sapphire" />
              {errors.name && <p className="text-xs text-red-500 mt-1">{errors.name}</p>}
            </div>
            {account && ( // Display these fields only when editing an existing account
              <>
                <div>
                  ...
                  <input type="text" value={account.accountNumber || 'N/A'} readOnly className="w-full px-3 py-2 rounded-lg transition-colors bg-slate-100 text-slate-500 border-transparent" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-600 mb-1">Expiry Date</label>
                  <input type="text" value={account.expiry || 'MM/YY'} readOnly className="w-full px-3 py-2 rounded-lg transition-colors bg-slate-100 text-slate-500 border-transparent" />
                </div>
              </>
            )}
          </>
         ...
         return (
          <>
            <div>
              ...
              <input type="text" name="name" value={currentAccount.name} onChange={handleInputChange} className={`w-full px-3 py-2 border rounded-lg transition-colors bg-transparent ${errors.name ? 'border-red-500' : 'border-slate-200'} ${!!account ? 'bg-slate-50 text-slate-500' : ''}`} placeholder="e.g., VTSAX Index Fund" />
              {errors.name && <p className="text-xs text-red-500 mt-1">{errors.name}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-600 mb-1">Investment Category</label>
              <select
      ...
      } else { // Bank accounts (Cash, Debit, Savings)
        return commonFields;
      }
    };
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md m-4">
        <div className="p-6 flex justify-between items-center">
          <h2 className="text-xl font-bold">{account ? 'Edit' : 'Add'} {selectedAccountType} Account</h2>
            ...
            <>
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setSelectedAccountType(e.target.value as AccountType | 'Investment' | 'Loan')}
              ...
              {selectedAccountType !== 'Cash' && ( // Only show bank branch for non-cash accounts
                <div>
                  <label className="block text-sm font-medium text-slate-600 mb-1">Bank Branch</label>
                  <select
