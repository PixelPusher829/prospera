...
const Analytics: React.FC = () => {
  ...
  const generateInsight = async () => {
    ...
    const advice = await getFinancialAdvice(MOCK_SUMMARY, EXPENSE_STATS, BALANCE_HISTORY);
    setInsight(advice);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-xl text-slate-600">
                <div className="w-1 h-4 bg-violet-200 rounded-full"></div> 50 transactions
              </div>
              <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-xl text-slate-600">
                <div className="w-1 h-4 bg-violet-400 rounded-full"></div> 15 categories
              </div>
            </div>
            <p className="text-xs text-slate-400">You have extra <span className="font-semibold text-green-600">$1,700</span> compared to last month</p>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col justify-between h-64">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-xl text-slate-600">
                <div className="w-1 h-4 bg-violet-200 rounded-full"></div> 27 transactions
              </div>
              <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-xl text-slate-600">
                <div className="w-1 h-4 bg-violet-400 rounded-full"></div> 6 categories
              </div>
            </div>
             <p className="text-xs text-slate-400">You earned extra <span className="font-semibold text-green-600">$500</span> compared to last month</p>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col justify-between h-64">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-xl text-slate-600">
                <div className="w-1 h-4 bg-violet-200 rounded-full"></div> 23 transactions
              </div>
              <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-xl text-slate-600">
                <div className="w-1 h-4 bg-violet-400 rounded-full"></div> 9 categories
              </div>
            </div>
             <p className="text-xs text-slate-400">You spent extra <span className="font-semibold text-red-600">$1,222</span> compared to last month</p>
          </div>
        </div>
      </div>
           ...
           <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 justify-center">
              {EXPENSE_STATS.map((stat, i) => (
                 <div key={i} className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] font-medium text-slate-600">{stat.name}</span>
                 </div>
              ))}
           </div>
        </div>
               ...
               <BarChart data={BUDGET_VS_EXPENSE} barGap={8}>
                ...
                />
                 <Tooltip 
                    ...
                    contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}
                 />
                 <Bar dataKey="value" fill="#8b5cf6" radius={[6, 6, 6, 6]} barSize={24} />
                 <Bar dataKey="value2" fill="#ddd6fe" radius={[6, 6, 6, 6]} barSize={24} />
               </BarChart>
             </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="flex-1 bg-white/10 rounded-2xl p-4 backdrop-blur-sm border border-white/10 mb-4 overflow-y-auto">
             {insight ? (
               ...
               <div className="h-full flex flex-col items-center justify-center text-center text-violet-200 text-sm">
                  <p>Get AI-powered insights based on your current financial metrics.</p>
               </div>
             )}
          </div>
