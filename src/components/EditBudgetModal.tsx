...
const EditBudgetModal: React.FC<EditBudgetModalProps> = ({ isOpen, onClose, onSave, initialBudget }) => {
  ...
  const handleCategoryChange = (id: string, field: keyof BudgetCategory, value: string | number) => {
    setBudget(prev => prev.map(cat => cat.id === id ? { ...cat, [field]: value } : cat));
  };
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl m-4">
        ...
        <div className="p-6 max-h-[60vh] overflow-y-auto space-y-4">
          {budget.map((category) => {
            ...
            return (
              <div key={category.id} className="grid grid-cols-14 gap-4 items-start">
                ...
                <div className="col-span-4">
                   <div className="relative">
                      ...
                      <input
                        ...
                        onChange={(e) => handleCategoryChange(category.id, 'limit', parseFloat(e.target.value) || 0)}
                        placeholder="Limit"
        ...
        <div className="p-6 bg-slate-50 rounded-b-2xl flex justify-end">
          <button 
            onClick={handleSave} 
            disabled={Object.values(errors).some(fieldErrors => Object.keys(fieldErrors).length > 0)}
