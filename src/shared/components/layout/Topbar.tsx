...
export default function Topbar({ onMenuClick }: { onMenuClick: () => void; isDarkMode: boolean }): React.FC{
		return (
			<header className="sticky top-0 z-20 flex items-center justify-between border-b border-slate-100 bg-white/80 px-6 py-4 backdrop-blur-md transition-colors lg:justify-end dark:border-slate-800 dark:bg-slate-900/80">
				...
				<div className="flex items-center gap-6">
					...
					<div className="flex items-center gap-4">
						<button type="button" title="Notifications" className="relative rounded-full p-2 text-slate-500 transition-colors hover:bg-slate-50 dark:text-slate-400 dark:hover:bg-slate-800">
							<Bell size={20} />
